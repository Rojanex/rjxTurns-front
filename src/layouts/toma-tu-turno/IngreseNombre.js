...
import ScanCC from "./components/ScanCC";
...
function IngreseNombre() {
  ...
  const handleButtonClickStep1 = (item) => {
    setNextStep(item);
    setStep(2);
  };
  ...
  const ReviewStep = ({ name, priority, fila, filaName }) => (
    <Card
      ...
      >
       ...
       <ConfirmComponent globalIP={globalIP} fila={fila} priority={priority} name={name} cc={cc}/>
      </MDBox>
    </Card>
  );
  ...
  const ChooseName = ({ fila, setFila, setStep }) => (
    <Card
      sx={{
        width: "fit-content",
        margin: "3rem",
    ...
    >
      ...
      <MDBox sx={{ display: "flex", justifyContent: "center" }}>
        <Button
          variant="contained"
          sx={{ mt: 5, mb: 5, fontSize: "1.7rem", color: "#FFFFFF" }}
          onClick={()  => handleButtonClickStep1("ScanCC")}
        >
          <MDTypography variant="" fontWeight="" sx={{ color: "black" }} />
          Escanee su cedula
        </Button>
      ...
      <MDBox sx={{ display: "flex", justifyContent: "center" }}>
        <Button
          variant="contained"
          sx={{ mt: 5, mb: 5, fontSize: "1.7rem", color: "#FFFFFF" }}
          onClick={()  => handleButtonClickStep1("NameStep")}
        >
          <MDTypography variant="" fontWeight="" sx={{ color: "black" }} />
          Escribir su nombre
        </Button>
  ...
  const renderStep2 = () => {
    switch(nextStep) {
      case "ScanCC":
        return <ScanCC name={name} setName={setName} setStep={setStep} cc={cc} setCC={setCC}/>;
      case "IngreseName":
        return <NameStep name={name} setName={setName} setStep={setStep} cc={cc} setCC={setCC}/>;
      default:
        return <NameStep name={name} setName={setName} setStep={setStep} cc={cc} setCC={setCC}/>;
    }
  }
  return (
    <CoverLayoutMonitor>
      <div>
        {step === 1 && (
          <ChooseName name={name} setName={setName} setStep={setStep} />
        )}
        {step === 2 && renderStep2()}
        {step === 3 && (
          <PriorityStep
        ...
        {step === 4 && <ChooseFila setFila={setFila} fila={fila} setStep={setStep}/>}
        {step === 5 && <ReviewStep name={name} priority={priority} filaName={filaName} fila={fila}/>}
      </div>
    </CoverLayoutMonitor>
  );
}
